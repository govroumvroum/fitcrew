...
// Composant assistant-ui vendorisé (registry `base/attachment`, flavor Base UI),
// localisé en français (textes visibles et libellés accessibles uniquement).
// Modifié : `uploadState` sans `message.submission` (voir plus bas) ;
// `ComposerAddAttachment` prend son libellé en prop et montre l'icône
// « image + » du chat (on n'accepte que des images) ; `useAttachmentSrc` vient de
// `./use-attachment-src`. Ne pas écraser via `assistant-ui add --overwrite` sans
// reporter la traduction et ces changements — voir AGENTS.md.
...
const AttachmentUI: FC = () => {
  ...
  // Modifié : le registry lit aussi `s.optional.message?.submission`, que
  // @assistant-ui/react 0.15.21 n'expose pas encore (erreur de type). Seul l'état
  // `running` de l'adaptateur compte donc comme « envoi en cours ».
  ...
  const errorMessage = useAuiState((s) =>
    s.attachment.status.type === "incomplete" &&
    s.attachment.status.reason === "error"
      ? (s.attachment.status.message ?? "L'envoi a échoué")
      : undefined,
  );
...
export const ComposerAddAttachment: FC<{ label: string }> = ({ label }) => {
  return (
    ...
    >
      <ImagePlusIcon className="aui-attachment-add-icon size-4" />
    </ComposerPrimitive.AddAttachment>
  );
};
